// 1. Definimos la interface según la API (usando campos opcionales ?)
interface LibroOL {
    title: string;
...
const contenedor = document.getElementById('resultadosLibros') as HTMLDivElement;
const errorMensaje = document.getElementById('errorBusqueda') as HTMLDivElement;
...
async function buscarLibros(query: string): Promise<void> {
    ...
    try {
        ...
        const libros: LibroOL[] = datos.docs.slice(0, 10); // Solo los primeros 10
